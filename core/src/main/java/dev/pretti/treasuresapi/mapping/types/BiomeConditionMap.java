...
public class BiomeConditionMap<T extends IConditionMap> implements IConditionMap
{
  private final HashMap<Biome, IConditionMap> whitelist = new HashMap<>();
   ...
   * Contrutor da classe
   */
  public BiomeConditionMap(Supplier<T> instanceSupplier)
  {
  ...
  public IConditionMap load(ITreasureProcessor processor)
  {
    ...
    for(ICondition condition : conditions)
      {
        if(condition instanceof IBiomeCondition)
          {
            IBiomeCondition biomeCondition = (IBiomeCondition) condition;
            if(biomeCondition.getAccessType() == EnumAccessType.WHITELIST)
              {
                for(Biome value : biomeCondition.getBiomes())
                  {
  ...
  public int evaluate(@NotNull IConditionMapContex context, int count)
  {
    ...
    Location loc   = context.getTreasureContext().getEventLocation();
    Biome    biome = loc.getBlock().getBiome();
    if(whitelist.containsKey(biome))
      {
        count = whitelist.get(biome).evaluate(context, count);
      }
   ...
   */
  public static <T extends IConditionMap> BiomeConditionMap<T> instance(Supplier<T> instanceSupplier)
  {
    return new BiomeConditionMap<>(instanceSupplier);
  }
}
