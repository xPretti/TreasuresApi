...
import dev.pretti.treasuresapi.collections.IListPicker;
import dev.pretti.treasuresapi.collections.LinearPicker;
import dev.pretti.treasuresapi.collections.RandomPicker;
...
public class TreasureProcessor implements ITreasureProcessor
{
    ...
    if(MathUtils.isChance(treasure.getChance())) {
      ...
      if(hasPermission(player.getPlayer(), treasure.getPermission())) {
        ...
        if(hasCondition(context, treasure.getConditions())) {
          ...
          limit = limit <= 0 ? values.size() : limit;
          while(picker.hasNext()) {
            if(count < limit) {
              rewardsGroup = picker.next();
  ...
  private boolean _processRewards(TreasureContext context, RewardsGroup rewardsGroup, int limit, boolean random) {
    if(rewardsGroup != null) {
      List<Rewards> values = rewardsGroup.getRewards();
      if(!values.isEmpty()) {
        boolean              wasRewarded = false;
        IListPicker<Rewards> picker      = random ? new RandomPicker<>(values) : new LinearPicker<>(values);
        int                  count       = 0;
        Rewards              rewards;
        limit = limit <= 0 ? values.size() : limit;
        Player player = context.getPlayer();
